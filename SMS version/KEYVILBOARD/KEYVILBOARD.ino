 ...
 //////////// Payload Method Make sure keyword are unique enough that subject in question wont enter them on their keyboard
  ...
  if (Serial.available()) {
    ...
    if (expect == "") {
      ...
      // parse result
      expect = "";
      String param = getValue(output,":",3);
      String lang = getValue(param,",",0);
      lang.trim();
      #ifdef DEBUG
        Serial.print(F("Got language: "));
        Serial.println(lang);
      #endif
      //USBhost.SetLang(lang);
    } else {         // wrong result
      expect = "";
      #ifdef DEBUG
        Serial.print(F("Error language: "));
